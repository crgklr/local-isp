...
def send_suggestions_email(report: dict) -> dict:
    ...
    subject = f"DWC Content Opportunities - {scan_date}"
    ...
    with httpx.Client(timeout=15) as c:
        resp = c.post("https://api.sendgrid.com/v3/mail/send",
                       ...
                       json={"personalizations": [{"to": [{"email": EMAIL_TO}]}],
                             "from": {"email": EMAIL_FROM, "name": "DWC Content Agent"},
                             "subject": subject,
...
<h2 style="color: #27ae60;">Article Draft Complete</h2>
...
The article has been published as a <strong>draft</strong> in Contentful. Log in to review, edit, and publish when ready.</p>
</body></html>"""
    text = f"Article Draft Complete: {article_title}\nSlug: {article_slug}\nContentful: {status} (ID: {entry_id})"
    if SENDGRID_API_KEY:
...
def poll_for_reply(since_minutes: int = 30) -> Optional[dict]:
    ...
    try:
        ...
        _, message_ids = mail.search(None, f'(FROM "{EMAIL_TO}" SUBJECT "Re: DWC Content Opportunities" SINCE {since_date})')
